...
KIR is a semantic compiler implementing a Knowledge ETL pipeline: it compiles raw sources into a canonical Knowledge Intermediate Representation (IR).
...
- Kir's job is to solve the "raw data" problem: compiling messy, inconsistent raw sources into a strict, machine-readable, versioned Knowledge IR that downstream tools can rely on without re-deriving structure themselves.
